...
const JobPostModal = () => {
    const { showModal, setShowModal, jobBeingEdited, setJobsData } =
        useAppDataContext()
        ...
        if (jobBeingEdited) {
            const formattedJobData = {
                ...
                applicationDeadline: new Date(
                    jobBeingEdited.applicationDeadline
                )
                    .toISOString()
                    .split("T")[0],
            }
    ...
    const getChangedFields = (current, original) => {
        ...
        if (current.companyName !== original.companyName)
            changes.companyName = current.companyName
        if (current.location !== original.location)
            changes.location = current.location
        if (current.jobType !== original.jobType)
            changes.jobType = current.jobType
        if (current.salaryMin !== original.salaryMin)
            changes.salaryMin = current.salaryMin
        if (current.salaryMax !== original.salaryMax)
            changes.salaryMax = current.salaryMax
        if (
            normalizeDate(current.applicationDeadline) !==
            normalizeDate(original.applicationDeadline)
        ) {
                            ...
                            <div className='bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4'>
                                <div className='sm:flex sm:items-start'>
                                    <div className='mt-3 text-center sm:mt-0 sm:text-left w-full'>
                                        <h3
                                            className='text-xl leading-6 font-medium text-gray-900 mb-6 text-center'
                                            id='modal-headline'
                                        >
                                            {jobBeingEdited
                                                ? "Edit Job Posting"
